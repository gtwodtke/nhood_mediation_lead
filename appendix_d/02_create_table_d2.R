sink("D:\\projects\\nhood_mediation_lead\\programs\\appendix_d\\_LOGS\\02_create_table_d2_log.txt")
print(Sys.time())

################################################
################################################
##                                            ##
## PROGRAM NAME: 02_create_table_d2           ##
## AUTHOR: GW                                 ##
## DATE: 5/25/2021                            ##
## DESCRIPTION:                               ##
##                                            ##
##  creates table of wave 2 nh effects        ##
##                                            ##
################################################
################################################
...
lasso.lambda1<-0.0146716
lasso.lambda2<-0.0121806
elast.alpha1<-0.5
elast.alpha2<-0.5
elast.lambda1<-0.0281389
elast.lambda2<-0.021286
...
m1.xvars<-c(bvars,c1vars,c2vars,"nsubage_3_r","ncondadvg_1","nbl5ug_1","ncondadvg_2")
m2.xvars<-c(bvars,c1vars,c2vars,"nsubage_3_r","ncondadvg_1","nbl5ug_1","ncondadvg_2","nbl5ug_2")
...
for (i in 1:nmi) {
	...
	phdcn$nsubage_3_r<-residuals(lm(nsubage_3~.,data=phdcn[,c(bvars,c1vars,c2vars,"ncondadvg_1","ncondadvg_2","nbl5ug_1","nbl5ug_2","nsubage_3")]))
	...
	for (j in 1:nboot) {
		...
		boot.phdcn$nsubage_3_r<-residuals(lm(nsubage_3~.,data=boot.phdcn[,c(bvars,c1vars,c2vars,"ncondadvg_1","ncondadvg_2","nbl5ug_1","nbl5ug_2","nsubage_3")]))
