sink("D:\\projects\\nhood_mediation_lead\\programs\\appendix_b\\_LOGS\\01_create_table_b1_log.txt")
print(Sys.time())

################################################
################################################
##                                            ##
## PROGRAM NAME: 01_create_table_b1           ##
## AUTHOR: GW                                 ##
## DATE: 5/25/2021                            ##
## DESCRIPTION:                               ##
##                                            ##
...
lasso.lambda1<-0.0164368
lasso.lambda2<-0.0124339
elast.alpha1<-0.5
elast.alpha2<-0.5
elast.lambda1<-0.0287238
elast.lambda2<-0.0217285

x1.vars<-c("npcage_b_r","npchsgrad_b_r","npcsomcol_b_r","npccolgrd_b_r","nownhome_b_r","nfemale_b_r","nblack_b_r","nhispan_b_r","nothrace_b_r","nfamsize_b_r",
	     "nsubage_3_r","nbl5ug_s_r","nlninc_s_r","npcemply_s_r","npcwelf_s_r","npcmarr_s_r","npcengl_s_r",
	     "ncondadvg_1","ncondadvg_2","ncondadvg_3")
x2.vars<-c("npcage_b_r","npchsgrad_b_r","npcsomcol_b_r","npccolgrd_b_r","nownhome_b_r","nfemale_b_r","nblack_b_r","nhispan_b_r","nothrace_b_r","nfamsize_b_r",
	     "nsubage_3_r","nlninc_s_r","npcemply_s_r","npcwelf_s_r","npcmarr_s_r","npcengl_s_r",
	     "ncondadvg_1","ncondadvg_2","ncondadvg_3","nbl5ug_1","nbl5ug_2","nbl5ug_3")
...
for (i in 1:nmi) {
	...
	# FIT MODELS #
	# BASE MODEL #
	m1.lm<-lm(nppvt_3~npcage_b_r+npchsgrad_b_r+npcsomcol_b_r+npccolgrd_b_r+nownhome_b_r+nfemale_b_r+nblack_b_r+nhispan_b_r+nothrace_b_r+nfamsize_b_r+
			   nsubage_3_r+nbl5ug_s_r+nlninc_s_r+npcemply_s_r+npcwelf_s_r+npcmarr_s_r+npcengl_s_r+
			   ncondadvg_1+ncondadvg_2+ncondadvg_3,data=phdcn)
	...
	m2.lm<-lm(nppvt_3~npcage_b_r+npchsgrad_b_r+npcsomcol_b_r+npccolgrd_b_r+nownhome_b_r+nfemale_b_r+nblack_b_r+nhispan_b_r+nothrace_b_r+nfamsize_b_r+
			   nsubage_3_r+nlninc_s_r+npcemply_s_r+npcwelf_s_r+npcmarr_s_r+npcengl_s_r+
			   ncondadvg_1+ncondadvg_2+ncondadvg_3+nbl5ug_1+nbl5ug_2+nbl5ug_3,data=phdcn)
	# LASSO #
	y<-phdcn[,"nppvt_3"]
	x1<-phdcn[,x1.vars]
	x2<-phdcn[,x2.vars]
	...
	gcomp<-phdcn
	gcomp$ncondadvg_1<-gcomp$ncondadvg_2<-gcomp$ncondadvg_3<-astar
	yhat.astar.lm<-predict(m1.lm,gcomp,type="response")
	yhat.astar.lasso<-predict(m1.lasso,newx=as.matrix(gcomp[,x1.vars]))
	yhat.astar.elast<-predict(m1.elast,newx=as.matrix(gcomp[,x1.vars]))
	gcomp$ncondadvg_1<-gcomp$ncondadvg_2<-gcomp$ncondadvg_3<-a
	yhat.a.lm<-predict(m1.lm,gcomp,type="response")
	yhat.a.lasso<-predict(m1.lasso,newx=as.matrix(gcomp[,x1.vars]))
	yhat.a.elast<-predict(m1.elast,newx=as.matrix(gcomp[,x1.vars]))
	...
	gcomp<-phdcn
	gcomp$ncondadvg_1<-gcomp$ncondadvg_2<-gcomp$ncondadvg_3<-astar
	gcomp$nbl5ug_1<-gcomp$nbl5ug_2<-gcomp$nbl5ug_3<-mstar
	yhat.astar.mstar.lm<-predict(m2.lm,gcomp,type="response")
	yhat.astar.mstar.lasso<-predict(m2.lasso,newx=as.matrix(gcomp[,x2.vars]))
	yhat.astar.mstar.elast<-predict(m2.elast,newx=as.matrix(gcomp[,x2.vars]))
	gcomp$ncondadvg_1<-gcomp$ncondadvg_2<-gcomp$ncondadvg_3<-a
	gcomp$nbl5ug_1<-gcomp$nbl5ug_2<-gcomp$nbl5ug_3<-m
	yhat.a.m.lm<-predict(m2.lm,gcomp,type="response")
	yhat.a.m.lasso<-predict(m2.lasso,newx=as.matrix(gcomp[,x2.vars]))
	yhat.a.m.elast<-predict(m2.elast,newx=as.matrix(gcomp[,x2.vars]))
	...
	yhat.astar.mfix.lasso<-predict(m2.lasso,newx=as.matrix(gcomp[,x2.vars]))
	yhat.astar.mfix.elast<-predict(m2.elast,newx=as.matrix(gcomp[,x2.vars]))
	gcomp$ncondadvg_1<-gcomp$ncondadvg_2<-gcomp$ncondadvg_3<-a
	yhat.a.mfix.lm<-predict(m2.lm,gcomp,type="response")
	yhat.a.mfix.lasso<-predict(m2.lasso,newx=as.matrix(gcomp[,x2.vars]))
	yhat.a.mfix.elast<-predict(m2.elast,newx=as.matrix(gcomp[,x2.vars]))
	...
	yhat.afix.mstar.lasso<-predict(m2.lasso,newx=as.matrix(gcomp[,x2.vars]))
	yhat.afix.mstar.elast<-predict(m2.elast,newx=as.matrix(gcomp[,x2.vars]))
	gcomp$nbl5ug_1<-gcomp$nbl5ug_2<-gcomp$nbl5ug_3<-m
	yhat.afix.m.lm<-predict(m2.lm,gcomp,type="response")
	yhat.afix.m.lasso<-predict(m2.lasso,newx=as.matrix(gcomp[,x2.vars]))
	yhat.afix.m.elast<-predict(m2.elast,newx=as.matrix(gcomp[,x2.vars]))
	...
	for (j in 1:nboot) {
		...
		boot.m1.lm<-lm(nppvt_3~npcage_b_r+npchsgrad_b_r+npcsomcol_b_r+npccolgrd_b_r+nownhome_b_r+nfemale_b_r+nblack_b_r+nhispan_b_r+nothrace_b_r+nfamsize_b_r+
					  nsubage_3_r+nbl5ug_s_r+nlninc_s_r+npcemply_s_r+npcwelf_s_r+npcmarr_s_r+npcengl_s_r+
					  ncondadvg_1+ncondadvg_2+ncondadvg_3,data=boot.phdcn)
		...
		boot.m2.lm<-lm(nppvt_3~npcage_b_r+npchsgrad_b_r+npcsomcol_b_r+npccolgrd_b_r+nownhome_b_r+nfemale_b_r+nblack_b_r+nhispan_b_r+nothrace_b_r+nfamsize_b_r+
					  nsubage_3_r+nlninc_s_r+npcemply_s_r+npcwelf_s_r+npcmarr_s_r+npcengl_s_r+
					  ncondadvg_1+ncondadvg_2+ncondadvg_3+nbl5ug_1+nbl5ug_2+nbl5ug_3,data=boot.phdcn)

		boot.y<-boot.phdcn[,"nppvt_3"]
		boot.x1<-boot.phdcn[,x1.vars]
		boot.x2<-boot.phdcn[,x2.vars]
		...
		boot.gcomp$ncondadvg_1<-boot.gcomp$ncondadvg_2<-boot.gcomp$ncondadvg_3<-astar
		boot.yhat.astar.lm<-predict(boot.m1.lm,boot.gcomp,type="response")
		boot.yhat.astar.lasso<-predict(boot.m1.lasso,newx=as.matrix(boot.gcomp[,x1.vars]))
		boot.yhat.astar.elast<-predict(boot.m1.elast,newx=as.matrix(boot.gcomp[,x1.vars]))
		boot.gcomp$ncondadvg_1<-boot.gcomp$ncondadvg_2<-boot.gcomp$ncondadvg_3<-a
		boot.yhat.a.lm<-predict(boot.m1.lm,boot.gcomp,type="response")
		boot.yhat.a.lasso<-predict(boot.m1.lasso,newx=as.matrix(boot.gcomp[,x1.vars]))
		boot.yhat.a.elast<-predict(boot.m1.elast,newx=as.matrix(boot.gcomp[,x1.vars]))
		...
		boot.gcomp<-boot.phdcn
		boot.gcomp$ncondadvg_1<-boot.gcomp$ncondadvg_2<-boot.gcomp$ncondadvg_3<-astar
		boot.gcomp$nbl5ug_1<-boot.gcomp$nbl5ug_2<-boot.gcomp$nbl5ug_3<-mstar
		boot.yhat.astar.mstar.lm<-predict(boot.m2.lm,boot.gcomp,type="response")
		boot.yhat.astar.mstar.lasso<-predict(boot.m2.lasso,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.yhat.astar.mstar.elast<-predict(boot.m2.elast,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.gcomp$ncondadvg_1<-boot.gcomp$ncondadvg_2<-boot.gcomp$ncondadvg_3<-a
		boot.gcomp$nbl5ug_1<-boot.gcomp$nbl5ug_2<-boot.gcomp$nbl5ug_3<-m
		boot.yhat.a.m.lm<-predict(boot.m2.lm,boot.gcomp,type="response")
		boot.yhat.a.m.lasso<-predict(boot.m2.lasso,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.yhat.a.m.elast<-predict(boot.m2.elast,newx=as.matrix(boot.gcomp[,x2.vars]))
		...
		boot.yhat.astar.mfix.lasso<-predict(boot.m2.lasso,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.yhat.astar.mfix.elast<-predict(boot.m2.elast,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.gcomp$ncondadvg_1<-boot.gcomp$ncondadvg_2<-boot.gcomp$ncondadvg_3<-a
		boot.yhat.a.mfix.lm<-predict(boot.m2.lm,boot.gcomp,type="response")
		boot.yhat.a.mfix.lasso<-predict(boot.m2.lasso,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.yhat.a.mfix.elast<-predict(boot.m2.elast,newx=as.matrix(boot.gcomp[,x2.vars]))
		...
		boot.yhat.afix.mstar.lasso<-predict(boot.m2.lasso,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.yhat.afix.mstar.elast<-predict(boot.m2.elast,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.gcomp$nbl5ug_1<-boot.gcomp$nbl5ug_2<-boot.gcomp$nbl5ug_3<-m
		boot.yhat.afix.m.lm<-predict(boot.m2.lm,boot.gcomp,type="response")
		boot.yhat.afix.m.lasso<-predict(boot.m2.lasso,newx=as.matrix(boot.gcomp[,x2.vars]))
		boot.yhat.afix.m.elast<-predict(boot.m2.elast,newx=as.matrix(boot.gcomp[,x2.vars]))
...
cat(" \n")
cat("===========================================\n")
cat("RWR with LASSO Regularization\n")
cat("===========================================\n")
...
cat(" \n")
cat("===========================================\n")
cat("RWR with Elasticnet Regularization\n")
cat("===========================================\n")
