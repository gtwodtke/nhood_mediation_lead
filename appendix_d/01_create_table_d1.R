sink("D:\\projects\\nhood_mediation_lead\\programs\\appendix_d\\_LOGS\\01_create_table_d1_log.txt")
print(Sys.time())
...
## DESCRIPTION:                               ##
##                                            ##
##  creates table of wave 1 nh effects        ##
##                                            ##
################################################
################################################
...
lasso.lambda1<-0.0186119
lasso.lambda2<-0.0186119
elast.alpha1<-0.5
elast.alpha2<-0.5
elast.lambda1<-0.0325248
elast.lambda2<-0.0325248
...
m1.xvars<-c(bvars,c1vars,"nsubage_3_r","ncondadvg_1")
m2.xvars<-c(bvars,c1vars,"nsubage_3_r","ncondadvg_1","nbl5ug_1")
...
for (i in 1:nmi) {
	...
	phdcn$nsubage_3_r<-residuals(lm(nsubage_3~.,data=phdcn[,c(bvars,c1vars,"ncondadvg_1","nbl5ug_1","nsubage_3")]))
	...
	for (j in 1:nboot) {
		...
		boot.phdcn$nsubage_3_r<-residuals(lm(nsubage_3~.,data=boot.phdcn[,c(bvars,c1vars,"ncondadvg_1","nbl5ug_1","nsubage_3")]))
